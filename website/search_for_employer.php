...
<link rel="stylesheet" href="style.css" type=text/css>
    ...
    <img id="logo" src="Logo.png">
        ...
        <title>Search for company scope</title>
    </head>
    <body>
        <?php 
            $name="";
            // isset () function is used to check whether a variable is set or not
            if(isset($_GET['company_scope']))
            $name = $_GET['company_scope'];
        ?>
        
        <form method="get">
        <h1>Search for an employer according to company's scope</h1   >
            <input class="sb" type='text' name="company_scope" value="<?php echo $name;?>">
            </br>
            </br>
        ...
        <?php
            if(isset($_GET['company_scope'])) {
                $sql = "SELECT* FROM employer
                    where `company_scope`='" .$_GET['company_scope']. "'"
